<template>
  <div class="cov-vue-date col-md-12">
  ...
  methods: {
    ...
    showDay (time) {
      ...
      for (let i = 1; i <= monthDays; i++) {
        ...
        if (i == Math.ceil(moment(currentMoment).format('D'))
          && moment(oldtime, this.option.format).year() == moment(currentMoment).year()
          && moment(oldtime, this.option.format).month() == moment(currentMoment).month())
        {
    ...
    checkBySelectDays (d, days) {
      this.selectedDays.forEach(day => {
        if (this.checked.year === moment(day).format('YYYY')
    ...
    getLimitCondition(limit, day) {
      if(limit.from && !limit.to) {
        return !moment(this.checked.year + '-' + this.checked.month
          + '-' + this.pad(day.value)).isAfter(limit.from)
      }else if(!limit.from && limit.to) {
        return !moment(this.checked.year + '-' + this.checked.month
          + '-' + this.pad(day.value)).isBefore(limit.to)
      }else {
        return !moment(this.checked.year + '-' + this.checked.month
          + '-' + this.pad(day.value)).isBetween(limit.from, limit.to)
      }
    },
    checkDay (obj) {
    ...
    picked() {
      if (this.option.type === 'day' || this.option.type === 'min') {
        let ctime = this.checked.year + '-' + this.checked.month
