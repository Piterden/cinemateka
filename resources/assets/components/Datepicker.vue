...
export default {
  ...
  methods: {
    ...
    update () {
      ...
      var value = this.value || this.$root.stringify(new Date());
      for (let i = 0; i < curDayCount; i++) {
        let tmpTime = new Date(time.getFullYear(), time.getMonth(), i + 1);
        let status = '';
        this.$root.stringify(tmpTime) === value && (status = 'date-active');
        arr.push({
          text: i + 1,
          time: tmpTime,
          status: status
    ...
    pickDate(index) {
      ...
      this.value = this.$root.stringify(this.now, 'YYYY-MM-DD')
